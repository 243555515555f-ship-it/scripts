...
GeminiLib.Themes = {
    ...
    VioletNebula = {
        ...
        Secondary = Color3.fromRGB(150, 90, 230),
...
function GeminiLib:CreateWindow(title, themeName)
    ...
    WindowObj.ColorPickerWindows = {} -- для отслеживания открытых пикеров
    ...
    -- Верхняя панель (Drag, Title, Stats, Close) — оставляем как было
    ...
    function WindowObj:CreateTab(name, icon)
        ...
        function Tab:Slider(text, min, max, default, callback, showValue)
            ...
            local function updateSlider(val, fromBox)
                ...
                -- Если мы в дробном режиме, округляем значение до 2 знаков после запятой ради красоты в UI
                if isFloat then
            ...
            local function onBarInput(input)
                ...
                local val
                if isFloat then
                    -- Для маленьких диапазонов (например 1-2) получаем точное дробное значение (1.23, 1.54 и т.д.)
                    val = min + (max - min) * pos
                else
                    -- Для больших диапазонов оставляем стандартное пошаговое округление
        ...
        -- УЛУЧШЕННЫЙ DROPDOWN (с поддержкой мульти-выбора)
...
-- Глобальная функция уведомления (оставлена для совместимости)
